...
- The local web chat can be light or dark. HTML iframes are transparent over the current agent-message surface (`rgba(255, 250, 241, 0.84)` on light; `rgba(23, 33, 43, 0.9)` on dark), so do not assume a fixed canvas. Prefer theme-independent HTML with explicit foreground/background contrast, or paint your own opaque `html, body` background full-bleed. If your HTML uses scripts, it may listen for `window.message` events of type `strix:theme` with `{ theme, background, text, muted, accent }` to adapt to theme changes; if that listener attaches asynchronously, call `window.strix.requestTheme()` afterward so the parent re-sends the current theme.
